/*
* @version: 1.2
* @Date:    2021-4-24 22:56:00
* @Last Modified time: 2021-5-26 18:08:43
* FunAdmin修改版
*/
//无限级下拉框
layui.define(['jquery', 'form'], function(exports){
    ...
    var obj = function(config){
		...
		if(this.config.url){
			this.config.data = this.getData(url);
		}
		//实例化配置
		...
		this.createSelect = function(optionData){
			...
			for(var i=0;i<optionData.length;i++){
				var disabled = optionData[i][c.status]==0 ? 'disabled="" ' : '';
				html+= '  <option '+disabled+'value="'+optionData[i][f[0]]+'">'+__(optionData[i][f[1]])+'</option>';
			}
			html+= ' </select>';
			html+= '</div>';
			return html;
		...
		this.getOptionData=function(catData,optionIndex){
			...
			for(var i=0;i<optionIndex.length;i++){
				if('undefined' == typeof catData[optionIndex[i]]){
					item = null;
					break;      
				}
				else if('undefined' == typeof catData[optionIndex[i]][this.config.children]){
					item = null;
					break;
				}
				else {
					item = catData[optionIndex[i]][this.config.children];
				}
			}
			if(!item){console.log(catData)
				item = this.getData(this.config.url+'?pid='+catData[optionIndex[0]][this.config.field[0]])
			}
			return item;
		};
		...
		this.change = function(elem){
			...
			num = $(c.elem).find('select[lay-filter="'+c.name+'"]').length;
			var childItem = o.getOptionData(c.data,index);
			if(childItem && num+1<=c.num){
				var html = o.createSelect(childItem);
				$thisItem.after(html);
				form.render('select',c.formFilter);
			}
	...
	obj.prototype.render = function(){
		...
		o.config.field  = typeof c.field==='string' ?c.field.split(','):c.field;
		if(typeof c.data==='' ||( typeof c.data==='object' && c.data.length==0)){

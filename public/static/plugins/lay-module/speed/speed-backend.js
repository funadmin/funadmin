layui.define(["jquery",'layer'], function (exports) {
    ...
    speedBackend = {
        ...
        render: function (options) {
            ...
            $.getJSON(options.initUrl, function (res) {
                if (res == null) {
                    ...
                    speedBackend.initLogo(res.logoInfo);
                    speedBackend.initMenu(res.menuInfo);
                    speedBackend.initBgColor();
                    speedBackend.initTabs({
                        filter: 'layui-layout-tabs',
                        maxTabs: options.maxTabs,
                        listenSwichCallback: function () {
                            speedBackend.initDevice();
                        }
                    });
                }
            }).fail(function () {
            ...
            speedBackend.hideLoading(options.loadingTime);
            speedBackend.listen();
        },

        //加载层,锁屏
        hideLoading:function(time=0.2){
            setTimeout(function () {
                ...
                if(layui.data('speedBackendLock').lock){
                    layer.prompt({
                        btn: ['立即解锁'],
                        title: ['屏幕锁定了,请输入解锁密码','background:'+bg+';color:#fff'],
                        closeBtn: 0,
                        formType: 1
                        ...
                        if (value.length < 1) {
                            ...
                            if(value == layui.data('speedBackendLock').lock){
                                layer.close(index);
                                //清除密码
                                layui.data('speedBackendLock', {
                                    key: 'lock'
                                    ,remove: true
                                });
                                layer.msg('解锁成功,欢迎回来!');
        ...
        initMenu: function (menuList) {
            ...
            $.each(menuList, function (key,val) {
                ...
                if (val.child!=[] && val.child!=undefined && val.child.length>0) {
                    child = val.child;
                    MenuHtml += '<a href="javascript:;" lay-id="'+val.id+'"  lay-tips="'+val.title+'" ><i class="' + val.icon + '"></i><cite> ' + val.title + '</cite> </a>';
                    var addChildHtml = function (html, child) {
                        html += '<dl class="layui-nav-child">\n';
                        $.each(child, function (k,v) {
                            html += '<dd>';
                            if (v.child!=[] && v.child!=undefined && v.child.length>0) {
                                html += '<a href="javascript:;" lay-id="'+ v.id +'"  lay-tips="'+v.title+'"><i class="' + v.icon + '"></i><cite> ' + v.title + '</cite></a>';
                                html = addChildHtml(html, v.child);
                            } else {
                                v.target = v.target?v.target:'_self';
                                html += '<a href="javascript:;" lay-id="'+ v.id +'" lay-tips="'+v.title+'" lay-href="' + v.href + '" target="' + v.target + '"><i class="' + v.icon + '"></i><cite> ' + v.title + '</cite></a>\n';
                            }
                            html += '</dd>\n';
                        });
                        html += '</dl>\n';
                    ...
                    MenuHtml += '<a href="javascript:;" lay-id="'+ val.id +'" lay-tips="'+val.title+'" lay-href="' + val.href + '" target="' + val.target + '"><i class="' + val.icon + '"></i><cite> ' + val.title + '</cite></a>\n';
                }
                MenuHtml += '</li>\n';
            });
        ...
        initTabs:function(options){
            ...
            speedBackend.listenScroll();
            speedBackend.listenSwitch(options);
            speedBackend.listenTabs();
            speedBackend.listenDeltab(options);
        },
        /**
         * 初始化logo
         * @param data
        ...
        delTab: function (layId, isParent) {
            ...
            speedBackend.changeSessioinTabId(layId)
        ...
        addTab: function (options) {
            ...
            speedBackend.changeSessioinTabId(options.layId)
            ...
            var checkLayId = speedBackend.checkLayId(options.layId);
            if (!checkLayId) {
       ...
       initBgColor :function () {
        ...
        listenSwitch: function (options) {
            ...
            element.on('tab(' + options.filter + ')', function (data) {
                ...
                speedBackend.changeSessioinTabId(layId)
                speedBackend.listenSwitchIframe(layId);
                speedBackend.scrollPostion();
            });
        },
        /**
         //  * 监听tab删除
        ...
        listen: function () {
            var events = layui.events = {
                ...
                opentheme: function(){
                    ...
                    var bgColorHtml = speedBackend.buildBgColorHtml();
                ...
                lockScreen:function(){
                    ...
                    layer.prompt({
                        btn: ['立即锁屏'],
                        title: ['设定密码可锁定屏幕','background:'+bg+';color:#fff'],
                        formType: 1
                    }, function (value, index, elem) {
                        if (value.length < 1) {
                            layer.msg('请先输入解锁密码');
                            ...
                            layer.prompt({
                                btn: ['解锁'],
                                title: ['屏幕已锁定,请输入解锁密码','background:'+bg+';color:#fff'],
                                closeBtn: 0,
                                formType: 1
                            }, function (value, index, elem) {
                                if (value.length < 1) {
                ...
                flexible: function (othis) {
                    speedBackend.sideFlexible();
                },
                rightPage:function(){
                    speedBackend.listenScroll('right')
                },
                leftPage :function(){
                    speedBackend.listenScroll('left')
                },
                ...
                , fullscreen: function (othis) {
                    ...
                    if (iconElem.hasClass(SCREEN_FULL)) {
                        speedBackend.fullScreen();
                        iconElem.addClass(SCREEN_REST).removeClass(SCREEN_FULL);
                    } else {
                        speedBackend.exitScreen();
                ...
                refresh: function (othis) {  //刷新
                    console.log(othis)
                    var url = othis.attr('lay-ajax') ?? speedBackend.refreshUrl;
    ...
    exports("speedBackend", speedBackend);
});
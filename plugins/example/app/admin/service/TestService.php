...
namespace app\admin\service\plugin\example;

use app\admin\model\plugin\example\Test;
use think\facade\Db;
...
final class TestService
{
  ...
  2 => 'field_3',
    ...
    public function assertNotReferenced(iterable $models, bool $force): ?string
    {
        ...
        if ($ids === []) return null;
        $references = Db::query('SELECT TABLE_NAME, COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE REFERENCED_TABLE_SCHEMA = DATABASE() AND REFERENCED_TABLE_NAME = ? AND REFERENCED_COLUMN_NAME = ?', ['example_test', 'id']);
        foreach ($references as $reference) {
            ...
            if (!preg_match('/^[a-z_][a-z0-9_]*$/', $table) || !preg_match('/^[a-z_][a-z0-9_]*$/', $column)) {
                throw new \RuntimeException(lang('test.referenceMetadataInvalid'));
            }
            if (Db::table($table)->whereIn($column, $ids)->limit(1)->count() > 0) {
                return lang('test.referenced', ['target' => $table . '.' . $column]);
            }
        }
        return null;
    }

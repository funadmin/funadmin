...
mkdir($pluginsPath . '/shop/app/admin', 0755, true);
mkdir($pluginsPath . '/worker/app/admin', 0755, true);
$records = [
...
foreach (['shop/products', '/admin/plugin/shop/orders'] as $path) {
...
foreach (['worker/task', '/admin/plugin/worker/run'] as $path) {
    $request = (new Request())->setPathinfo($path);
    guardExpect(guardStatus(fn () => $guard->handle($request, $next)) === 404, '已知禁用插件路径必须返回 404：' . $path);
}
foreach (['admin/system/index', 'api/v2/token', 'index/home', 'install/index', 'common/internal', 'news/article', 'unknown/path'] as $path) {
    $request = (new Request())->setPathinfo($path);
    guardExpect(guardStatus(fn () => $guard->handle($request, $next)) === 200, '核心或未知普通路径必须放行：' . $path);
}
$request = (new Request())->setPathinfo('admin/plugin/unknown/action');
...
$request = (new Request())->setPathinfo('admin/plugin/shop/orders');

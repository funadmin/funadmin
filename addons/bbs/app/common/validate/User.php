...
{
    protected $rule = [
        'username|用户名' => 'require|min:2|max:18|unique:user|chsDash',
    ...
    {
        return $this->only(['username','password','vercode'])
    ...
    {
        return $this->only(['email','password','vercode'])
			->remove('email', 'unique');
    }
    ...
    {
        return $this->only(['email','vercode'])
				->remove('email', 'unique');
    }
	...
	{
		return $this->only(['email','nickname','city','sex','sign'])
					->remove('email','unique');
	}

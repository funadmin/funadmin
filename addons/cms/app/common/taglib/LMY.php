...
// | lemocms [ 御宅男工作室 ]
// +----------------------------------------------------------------------
// | Copyright (c) 2018 http://lemocms.com All rights reserved.
// +----------------------------------------------------------------------
...
<?php
   ...
   \$tableModel = \\think\\facade\\Db::name(strtolower('{$table}'));
    if(!empty('{$where}')){
            \$data =  \$tableModel->whereRaw('{$where}')->whereRaw('status=1')->cache({$cache})->field('{$field}')->order('{$order}')->paginate(['list_rows' => {$num} ,'page' =>{$page}]);
    }else{
        \$data =  \$tableModel->whereRaw('status=1')->order('{$order}')->field('{$field}')->cache({$cache})->paginate(['list_rows' => {$num} ,'page' =>{$page}]);
...
<?php
    ...
    \$cacheID = \\lemo\\helper\\StringHelper::uuid('md5','{$tagString}');

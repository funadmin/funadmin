-- 014 附件存储驱动：补充对象键、扩展路径长度并修正上传配置。

ALTER TABLE `fun_attach`
  ADD COLUMN `storage_key` varchar(1024) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT '' COMMENT '存储对象键' AFTER `name`,
  MODIFY COLUMN `thumb` varchar(1024) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '缩略图',
  MODIFY COLUMN `path` varchar(1024) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '访问路径',
...
INSERT IGNORE INTO `fun_auth_rule`
(`id`, `pid`, `app`, `route_name`, `controller`, `action`, `title`, `resource_type`, `status`, `is_public`, `sort`, `source_type`, `source_name`, `create_time`, `delete_time`)
VALUES

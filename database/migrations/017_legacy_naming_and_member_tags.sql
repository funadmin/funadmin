...
INSERT INTO `fun_language` (`id`,`name`,`is_default`,`status`,`create_time`,`update_time`,`delete_time`)
SELECT `id`,`name`,`is_default`,`status`,COALESCE(`create_time`,0),COALESCE(`update_time`,0),`delete_time` FROM `fun_languages`
ON DUPLICATE KEY UPDATE `name`=VALUES(`name`),`is_default`=VALUES(`is_default`),`status`=VALUES(`status`),`update_time`=VALUES(`update_time`),`delete_time`=VALUES(`delete_time`);
...
INSERT INTO `fun_region` (`id`,`pid`,`name`,`short_name`,`area_code`,`zip_code`,`pinyin`,`longitude`,`latitude`,`level`,`sort`,`delete_time`)
SELECT `id`,`pid`,`name`,`short_name`,CAST(`areacode` AS CHAR),CAST(`zipcode` AS CHAR),`pinyin`,NULLIF(`lng`,''),NULLIF(`lat`,''),`level`,`sort`,`delete_time` FROM `fun_provinces`
ON DUPLICATE KEY UPDATE `pid`=VALUES(`pid`),`name`=VALUES(`name`),`short_name`=VALUES(`short_name`),`area_code`=VALUES(`area_code`),`zip_code`=VALUES(`zip_code`),`pinyin`=VALUES(`pinyin`),`longitude`=VALUES(`longitude`),`latitude`=VALUES(`latitude`),`level`=VALUES(`level`),`sort`=VALUES(`sort`),`delete_time`=VALUES(`delete_time`);
...
CREATE TABLE IF NOT EXISTS `fun_member_tag` (
  ...
  KEY `idx_member_tag_status_sort` (`status`,`sort`)
...
INSERT IGNORE INTO `fun_member_tag` (`name`,`status`,`sort`,`create_time`,`update_time`)
SELECT DISTINCT TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(m.`tags`, ',', seq.n), ',', -1)),1,0,UNIX_TIMESTAMP(),UNIX_TIMESTAMP()

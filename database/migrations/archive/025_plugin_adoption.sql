...
SET @sql = IF(
  ...
  CONCAT(
    'INSERT INTO `', @plugin_table_name, '` (`title`,`name`,`description`,`author`,`version`,`requires`,`website`,`config`,`status`,`lifecycle_state`,`needs_reinstall`,`created_at`,`updated_at`) ',
    ...
    ',0,''disabled'',1,FROM_UNIXTIME(NULLIF(legacy.`create_time`,0)),FROM_UNIXTIME(NULLIF(legacy.`update_time`,0)) FROM `', @legacy_table_name, '` legacy ',
...
-- 007 可能已将唯一的 fun_addon 整表改名为 fun_plugin；无新 manifest 的记录仍视为旧来源。
SET @sql = IF(
  @plugin_exists,
  CONCAT('UPDATE `', @plugin_table_name, '` SET `status` = 0,`lifecycle_state` = ''disabled'',`needs_reinstall` = 1 WHERE `manifest` IS NULL'),
  'SELECT 1'
);
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;

...
installHttp.interceptors.response.use(
  (response) => {
    ...
    return Promise.reject(new Error(payload.msg || i18n.global.t('install.requestFailed', '安装请求失败')));
  },
  (error) => {
    // 后端 fail() 走 HTTP 状态码：优先透出业务 msg，避免 axios 通用报错淹没原因
    const payload = error?.response?.data as API.Response | undefined;
    return Promise.reject(new Error(payload?.msg || error?.message || i18n.global.t('install.requestFailed', '安装请求失败')));
  }
);

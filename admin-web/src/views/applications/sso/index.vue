...
const modes: Array<{ value: 'off'|'native'|'external'; title: string; description: string; disabled: boolean }> = [{ value: 'off', title: '关闭 SSO', description: '保留本地登录，不提供统一身份入口', disabled: false }, { value: 'native', title: '身份提供方', description: 'FunAdmin 作为 OIDC Provider', disabled: false }, { value: 'external', title: '外部身份接入', description: '连接企业外部 IdP', disabled: true }];
const form = reactive<SsoConfig>({ enabled: false, providerMode: 'native', issuer: '', externalIdentityEnabled: false, backchannelLogoutEnabled: true });
...
const select = (mode: 'off'|'native'|'external') => { form.enabled = mode !== 'off'; if (mode !== 'off') form.providerMode = mode; };

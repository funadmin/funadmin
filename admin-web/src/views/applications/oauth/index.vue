<template>
  <PageWrapper title="OAuth Client" subtitle="管理应用的 OAuth/OIDC client、redirect URI、scope、grant、secret 与签名 key">
    <div class="mb-4 flex flex-wrap gap-2"><el-input v-model="keyword" class="max-w-80" placeholder="搜索名称或 Client ID" clearable @keyup.enter="load"/><el-button type="primary" plain @click="load">搜索</el-button><el-button type="success" plain @click="openCreate">新建 Client</el-button><el-button @click="openKeys">签名 Key</el-button></div>
    <el-table v-loading="loading" :data="clients"><el-table-column prop="name" label="名称"/><el-table-column prop="clientId" label="Client ID" min-width="260"/><el-table-column prop="clientType" label="类型"/><el-table-column prop="status" label="状态"/><el-table-column label="操作" width="330"><template #default="scope"><el-button @click="edit(scope.row as OAuthClient)">配置</el-button><el-button :disabled="scope.row.clientType === 'public' || scope.row.status !== 'active'" @click="createSecret(scope.row as OAuthClient)">创建 Secret</el-button><el-button type="warning" plain :disabled="scope.row.status !== 'active'" @click="disable(scope.row as OAuthClient)">禁用</el-button><el-button type="danger" plain @click="remove(scope.row as OAuthClient)">删除</el-button></template></el-table-column></el-table>
    ...
    <el-drawer v-model="keysVisible" title="OIDC 签名 Key" size="620px"><el-button type="primary" plain class="mb-4" @click="rotateKey">轮换 Key</el-button><el-table :data="keys"><el-table-column prop="kid" label="KID"/><el-table-column prop="status" label="状态"/><el-table-column prop="publishUntil" label="旧 Key 发布至"/></el-table></el-drawer>
  </PageWrapper>
</template>

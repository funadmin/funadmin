    ...
    <el-tabs v-model="mode">
      <el-tab-pane label="创建插件" name="create">
        <el-form :model="createForm" label-width="110px">
          <el-form-item label="name"><el-input v-model="createForm.name" placeholder="小写字母开头，仅字母和数字" /></el-form-item>
          <el-form-item label="title"><el-input v-model="createForm.title" /></el-form-item>
          <el-form-item label="application"><el-switch v-model="createForm.application" /></el-form-item>
          <el-form-item label="console"><el-switch v-model="createForm.console" /></el-form-item>
      ...
      <el-tab-pane label="生成 CRUD" name="crud">
        <el-form :model="crudForm" label-width="110px">
          ...
          <el-form-item label="table"><el-select v-model="crudForm.table" filterable class="w-full"><el-option v-for="item in tables" :key="item.name" :label="item.comment ? `${item.name} — ${item.comment}` : item.name" :value="item.name" /></el-select></el-form-item>
          <el-form-item label="entity"><el-input v-model="crudForm.entity" /></el-form-item>
          <el-form-item label="scope"><el-select v-model="crudForm.scope" class="w-full"><el-option v-for="scope in selectedPlugin?.scopes || []" :key="scope" :label="scope" :value="scope" /></el-select></el-form-item>
        </el-form>
        ...
        <el-table v-if="crudPreview" :data="crudPreview.plan.files" size="small" max-height="220"><el-table-column prop="path" label="目标文件" /><el-table-column prop="status" label="状态" width="90" /></el-table>
...
async function createPlugin() { await run(async () => { result.value = await pluginDevelopmentApi.create({ ...createForm }); ElMessage.success(`插件已创建，auditId：${result.value.auditId}`); preview.value = null; await loadOptions(); emit('changed'); }); }
...
async function generateCrud() { if (!crudDefinition.value || !confirmToken.value) return; await run(async () => { const generated = await crudDevelopmentApi.generate(crudDefinition.value!, confirmToken.value, []); confirmToken.value = ''; ElMessage.success(`CRUD 已生成，auditId：${generated.generationId}`); emit('changed'); }); }
</script>

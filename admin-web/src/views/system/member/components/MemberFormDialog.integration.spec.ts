...
const definition = JSON.parse(execFileSync(process.env.PHP_BINARY || 'php', ['-r', `require 'vendor/autoload.php'; echo json_encode(\\app\\admin\\service\\MemberFormDefinition::build(['groups'=>[['id'=>7,'name'=>'会员组']], 'levels'=>[['id'=>3,'name'=>'会员等级']], 'tags'=>[['id'=>5,'name'=>'标签']]]));`], { cwd: resolve(process.cwd(), '..'), encoding: 'utf8' }));

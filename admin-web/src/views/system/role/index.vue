      ...
      <main class="role-workspace__detail">
        ...
        <template v-else>
          ...
          <el-tabs v-model="activeTab">
            ...
            <el-tab-pane v-if="canManageAuthorization" label="功能权限" name="permissions" /><el-tab-pane v-if="canManageAuthorization" label="字段权限" name="fields" /><el-tab-pane v-if="canManageAuthorization" label="数据授权" name="data" /><el-tab-pane label="继承关系" name="inheritance"><div class="inheritance-info"><p>主父级：{{ parentName(activeRole?.parentId) }}</p><p>额外继承：{{ inheritedNames }}</p></div></el-tab-pane>
          </el-tabs>
          <RoleAuthorizationPanel v-if="canManageAuthorization && activeRole && ['permissions', 'fields', 'data'].includes(activeTab)" :key="`${activeRole.id}-${activeTab}`" :role-id="activeRole.id" :section="activeTab as 'permissions' | 'fields' | 'data'" @success="loadRoles(activeRole.id)" />
        </template>
      </main>
    </div>
...
const activeRole = ref<RoleModel>(); const mode = ref<Mode>('view'); const activeTab = ref('basic'); const treeRef = ref<{ setCurrentKey: (key?: number) => void; store?: { nodesMap?: Record<string, { expanded: boolean }> } }>(); const formRef = ref<FormInstance>();
...
const inheritedNames = computed(() => form.parentRoleIds.map((id) => roles.value.find((role) => role.id === id)?.name).filter(Boolean).join('、') || '无');
...
async function loadRoles(selectedId?: number) { loading.value = true; try { roles.value = await roleApi.all(); const selected = roles.value.find((role) => role.id === selectedId) || roles.value[0]; activeRole.value = selected; if (mode.value === 'view') fillForm(selected); treeRef.value?.setCurrentKey(selected?.id); } finally { loading.value = false; } }
...
function selectRole(role: RoleTreeNode) { activeRole.value = role; mode.value = 'view'; activeTab.value = 'basic'; fillForm(role); contextVisible.value = false; }
...
function runContext(action: 'detail' | 'edit' | 'add' | 'permission' | 'copy' | 'status' | 'delete') { const role = contextRole.value; contextVisible.value = false; if (!role) return; if (action === 'detail') selectRole(role as RoleTreeNode); else if (action === 'edit') startEdit(role); else if (action === 'add') startCreate(role); else if (action === 'permission' || action === 'copy') { activeRole.value = role; mode.value = 'view'; activeTab.value = 'permissions'; fillForm(role); } else if (action === 'status') void toggleStatus(role); else void removeRole(role); }
function parentName(parentId?: number) { return roles.value.find((role) => role.id === parentId)?.name || '顶级角色'; }

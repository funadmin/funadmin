<template>
  <PageWrapper :title="t('systemRole.title', '角色管理')" :subtitle="t('systemRole.subtitle', '维护角色层级、基本信息与授权')">
    <div class="role-workspace">
      <aside class="role-tree-card role-workspace__tree">
        <div class="role-tree-card__header">
          <div>
            <h3>{{ t('systemRole.catalog', '角色目录') }}</h3>
            <span>{{ t('systemRole.roleCount', { n: roles.length }, '{n} 个角色') }}</span>
          </div>
          <el-button type="primary" link v-perm="'system:role:add'" @click="startCreate()"><i class="i-ep-plus" /> {{ t('common.add', '新增') }}</el-button>
        </div>
        <el-input v-model="keyword" :placeholder="t('systemRole.searchPlaceholder', '搜索角色名称或标识')" clearable class="role-tree-card__search" />
        <el-tree ref="treeRef" :data="filteredTree" node-key="id" :props="{ label: 'name', children: 'children' }" highlight-current default-expand-all :current-node-key="activeRole?.id" @node-click="selectRole" @node-contextmenu="openContextMenu">
          <template #default="{ data }"><span class="role-node"><span>{{ data.name }}</span><el-tag :type="data.status === 1 ? 'success' : 'info'" size="small">{{ data.status === 1 ? t('common.enable', '启用') : t('common.disable', '禁用') }}</el-tag></span></template>
        </el-tree>
      </aside>
      ...
      <main class="role-detail-card role-workspace__detail">
        <div class="role-workspace__toolbar">
          <div class="role-toolbar__actions">
            <el-button @click="setExpanded(true)"><i class="i-ep-expand" /> {{ t('systemRole.expandAll', '展开全部') }}</el-button>
            <el-button @click="setExpanded(false)"><i class="i-ep-fold" /> {{ t('systemRole.collapseAll', '折叠全部') }}</el-button>
            <el-button :loading="loading" @click="loadRoles(activeRole?.id)"><i class="i-ep-refresh" /> {{ t('common.refresh', '刷新') }}</el-button>
          </div>
        </div>
        <el-empty v-if="!activeRole && mode === 'view'" :description="t('systemRole.emptyHint', '请选择角色或添加顶级角色')" />
        <template v-else>
          <header class="role-detail__header">
            <div><h3>{{ mode === 'create' ? t('systemRole.dialogAdd', '新增角色') : activeRole?.name }}</h3><span>{{ mode === 'edit' ? t('systemRole.editingBasic', '正在编辑基本信息') : t('systemRole.workspace', '角色配置工作区') }}</span></div>
            <el-button v-if="activeRole && mode === 'view'" v-perm="'system:role:edit'" @click="startEdit(activeRole)">{{ t('systemRole.editRole', '修改角色') }}</el-button>
          </header>
          <el-tabs v-model="activeTab">
            <el-tab-pane :label="t('systemRole.tabBasic', '基本信息')" name="basic">
              <el-form ref="formRef" :model="form" :rules="rules" label-width="100px" class="role-form">
                <el-row :gutter="16"><el-col :span="12"><el-form-item :label="t('systemRole.name', '名称')" prop="name"><el-input v-model="form.name" :disabled="mode === 'view'" /></el-form-item></el-col><el-col :span="12"><el-form-item :label="t('systemRole.code', '标识')" prop="code"><el-input v-model="form.code" :disabled="mode !== 'create'" /></el-form-item></el-col></el-row>
                <el-row :gutter="16"><el-col :span="12"><el-form-item :label="t('systemRole.level', '角色等级')" prop="level"><el-input-number v-model="form.level" :disabled="mode === 'view'" :min="1" :max="9999" class="w-full" /></el-form-item></el-col><el-col :span="12"><el-form-item :label="t('common.status', '状态')"><el-radio-group v-model="form.status" :disabled="mode === 'view'"><el-radio-button :value="1">{{ t('common.enable', '启用') }}</el-radio-button><el-radio-button :value="0">{{ t('common.disable', '禁用') }}</el-radio-button></el-radio-group></el-form-item></el-col></el-row>
                <el-form-item :label="t('systemRole.primaryParent', '主父级')" prop="parentId"><el-tree-select v-model="form.parentId" :data="parentOptions" :props="{ label: 'name', children: 'children' }" node-key="id" check-strictly clearable :disabled="mode === 'view'" :placeholder="t('systemRole.topRole', '顶级角色')" class="w-full" /><div class="form-tip">{{ t('systemRole.primaryParentTip', '主父级决定角色树中的位置，可在新增子角色后调整') }}</div></el-form-item>
                <el-form-item :label="t('systemRole.additionalInherit', '额外继承')"><el-tree-select v-model="form.parentRoleIds" :data="additionalParentOptions" :props="{ label: 'name', children: 'children' }" node-key="id" multiple show-checkbox check-strictly clearable :disabled="mode === 'view'" class="w-full" /></el-form-item>
                <el-form-item :label="t('systemRole.dataScope', '数据范围')" prop="dataScope"><el-select v-model="form.dataScope" :disabled="mode === 'view'" class="w-full"><el-option :label="t('systemRole.scopeAll', '全部数据')" value="all" /><el-option :label="t('systemRole.scopeDeptAndChildren', '本部门及下级')" value="dept_and_children" /><el-option :label="t('systemRole.scopeDept', '本部门')" value="dept" /><el-option :label="t('systemRole.scopeSelf', '仅本人')" value="self" /><el-option :label="t('systemRole.scopeCustom', '自定义部门')" value="custom" :disabled="!can('system:dept:list')" /></el-select></el-form-item>
                <el-form-item v-if="form.dataScope === 'custom'" :label="t('systemRole.scopeCustom', '自定义部门')" prop="departmentIds"><div class="w-full"><el-alert v-if="!can('system:dept:list')" :title="t('systemRole.noDeptPerm', '无部门查看权限，无法配置自定义部门')" type="warning" :closable="false" show-icon class="mb-2" /><el-tree-select v-model="form.departmentIds" :data="departmentOptions" :props="{ label: 'name', children: 'children' }" node-key="id" multiple show-checkbox check-strictly collapse-tags collapse-tags-tooltip clearable :disabled="mode === 'view' || !can('system:dept:list')" :loading="departmentsLoading" :placeholder="t('systemRole.selectDept', '请选择部门')" class="w-full" /></div></el-form-item>
                <el-form-item :label="t('systemRole.remark', '备注')"><el-input v-model="form.remark" type="textarea" :rows="3" :disabled="mode === 'view'" /></el-form-item>
                <div v-if="mode !== 'view'" class="role-form__actions"><el-button @click="cancelEdit">{{ t('common.cancel', '取消') }}</el-button><el-button type="primary" :loading="saving" v-perm="mode === 'create' ? 'system:role:add' : 'system:role:edit'" @click="saveRole">{{ t('common.save', '保存') }}</el-button></div>
              </el-form>
            </el-tab-pane>
            <el-tab-pane v-if="canManageAuthorization" :label="t('systemRole.tabPermissions', '功能权限')" name="permissions">
              <el-alert v-if="authorizationError" :title="authorizationError" type="error" :closable="false" show-icon><el-button link type="primary" @click="reloadAuthorization">{{ t('systemRole.reload', '重新加载') }}</el-button></el-alert>
            ...
            <el-tab-pane v-if="canManageAuthorization" :label="t('systemRole.tabFields', '字段权限')" name="fields">
            ...
            <el-tab-pane v-if="canManageAuthorization" :label="t('systemRole.tabData', '数据授权')" name="data">
            ...
            <el-tab-pane v-if="canManageAuthorization" :label="t('systemRole.tabInheritance', '继承关系')" name="inheritance">
              <div class="inheritance-info">
                <section><h4>{{ t('systemRole.primaryParent', '主父级') }}</h4><el-tag v-if="primaryParent" type="primary">{{ primaryParent.roleName }}</el-tag><span v-else>{{ t('systemRole.topRole', '顶级角色') }}</span></section>
                <section><h4>{{ t('systemRole.additionalParents', '额外父级') }}</h4><div v-if="additionalParents.length" class="inheritance-tags"><el-tag v-for="parent in additionalParents" :key="parent.roleId" type="info">{{ parent.roleName }}</el-tag></div><span v-else>{{ t('systemRole.none', '无') }}</span></section>
                <section><h4>{{ t('systemRole.ancestorChain', '有效祖先链 / 继承来源') }}</h4><el-timeline v-if="inheritanceAncestors.length"><el-timeline-item v-for="ancestor in inheritanceAncestors" :key="ancestor.roleId"><strong>{{ ancestor.roleName }}</strong><span class="inheritance-source">{{ t('systemRole.sourceFrom', { names: ancestor.sourceRoleNames.join('、') }, '来源：{names}') }}</span></el-timeline-item></el-timeline><span v-else>{{ t('systemRole.noAncestors', '无传递祖先') }}</span></section>
              </div>
            </el-tab-pane>
          </el-tabs>
        </template>
    ...
    <Teleport to="body"><div v-if="contextVisible" class="role-context-menu" :style="{ left: `${contextPosition.x}px`, top: `${contextPosition.y}px` }" @click.stop>
      <button @click="runContext('detail')">{{ t('systemRole.viewDetail', '查看详情') }}</button><button v-if="can('system:role:edit')" @click="runContext('edit')">{{ t('systemRole.editRole', '修改角色') }}</button><button v-if="can('system:role:add')" @click="runContext('add')">{{ t('systemRole.addChild', '添加子角色') }}</button><button v-if="can('system:role:perm')" data-context-action="permission" @click="runContext('permission')">{{ t('systemRole.configPerm', '配置权限') }}</button><button v-if="can('system:role:perm') && can('system:role:perm-copy')" @click="runContext('copy')">{{ t('systemRole.copyAuth', '复制授权') }}</button><button v-if="can('system:role:edit')" @click="runContext('status')">{{ contextRole?.status === 1 ? t('common.disable', '禁用') : t('common.enable', '启用') }}</button><button v-if="can('system:role:delete')" class="danger" @click="runContext('delete')">{{ t('common.remove', '删除') }}</button>
    </div></Teleport>
  </PageWrapper>
</template>
...
const rules = computed<FormRules>(() => ({ name: [{ required: true, message: t('systemRole.nameRequired', '请输入名称'), trigger: 'blur' }], code: [{ required: true, message: t('systemRole.codeRequired', '请输入标识'), trigger: 'blur' }, { pattern: /^[a-zA-Z][a-zA-Z0-9_]*$/, message: t('systemRole.codePattern', '需以英文开头，仅支持字母、数字和下划线'), trigger: 'blur' }], level: [{ required: true, message: t('systemRole.levelRequired', '请输入角色等级'), trigger: 'change' }], dataScope: [{ required: true, message: t('systemRole.dataScopeRequired', '请选择数据范围'), trigger: 'change' }], departmentIds: [{ validator: (_rule, value: number[], callback) => { if (form.dataScope === 'custom' && (!value || value.length === 0)) callback(new Error(t('systemRole.customDeptRequired', '请选择自定义部门'))); else callback(); }, trigger: 'change' }] }));
...
async function ensureAuthorization(roleId: number) {
  ...
  try {
    ...
    authorizationError.value = error instanceof Error ? error.message : t('systemRole.authLoadFailed', '授权详情加载失败');
  }
}
...
async function removeRole(role: RoleModel) { await ElMessageBox.confirm(t('systemRole.deleteConfirm', { name: role.name }, '确认删除角色 {name}？此操作不可恢复'), t('systemRole.deleteTitle', '删除确认'), { type: 'warning' }); await roleApi.remove(role.id); mode.value = 'view'; await loadRoles(); }
async function toggleStatus(role: RoleModel) { await ElMessageBox.confirm(t('systemRole.statusConfirm', { action: role.status === 1 ? t('common.disable', '禁用') : t('common.enable', '启用'), name: role.name }, '确认{action}角色 {name}？'), t('systemRole.statusTitle', '状态确认'), { type: 'warning' }); const saved = await roleApi.update(role.id, { ...role, status: role.status === 1 ? 0 : 1 }); await loadRoles(saved.id); }

  ...
  <div v-loading="loading" class="role-authorization-panel">
    <div class="role-authorization-panel__toolbar">
      ...
      <div class="flex gap-2">
        ...
        <el-button :disabled="!copySourceRoleId" :loading="copying" v-perm="'system:role:perm-copy'" @click="copyAuthorization">{{ t('systemRole.copyAuth', '复制授权') }}</el-button>
      </div>
    </div>
        ...
        <el-collapse v-model="expandedGroups">
          <el-collapse-item v-for="group in permissionGroups" :key="group.id" :name="group.id">
            ...
            <el-table :data="group.resources" border>
              <el-table-column prop="name" :label="t('systemRole.resource', '资源')" min-width="180" />
              <el-table-column :label="t('systemRole.actions', '动作')" min-width="520">
                <template #default="{ row: resource }">
                  <div class="resource-actions__header">
                    <span class="resource-actions__hint">{{ t('systemRole.actionCount', { n: resource.actions.length }, '{n} 项动作') }}</span>
                    <div class="resource-actions__tools">
                      <el-button link type="primary" size="small" :disabled="!hasDirectActions(resource as PermissionResourceRow)" @click="toggleResourceActions(resource as PermissionResourceRow, true)">{{ t('systemRole.selectAll', '全选') }}</el-button>
                      <el-button link type="primary" size="small" :disabled="!hasDirectActions(resource as PermissionResourceRow)" @click="toggleResourceActions(resource as PermissionResourceRow, false)">{{ t('systemRole.deselectAll', '反选') }}</el-button>
                    </div>
                  </div>
                  <div class="flex flex-wrap gap-x-6 gap-y-2">
                    <el-tooltip v-for="action in resource.actions" :key="action.id" :content="inheritanceText(action.inheritedFrom)" :disabled="!action.inherited">
                      <el-checkbox :model-value="action.direct || action.inherited" :disabled="action.inherited" @change="setPermission(action, $event)">
                        {{ action.name }}<el-tag v-if="action.inherited" size="small" type="info">{{ t('systemRole.inherited', '继承') }}</el-tag>
                      </el-checkbox>
                    </el-tooltip>
                  </div>
                </template>
    ...
    <div v-else-if="section === 'fields'">
        <el-table :data="fields" border row-key="id">
          ...
          <el-table-column prop="name" :label="t('systemRole.field', '字段')" min-width="180"><template #default="{ row }">{{ row.name }} <span class="field-code">{{ row.field }}</span></template></el-table-column>
          <el-table-column :label="t('systemRole.view', '查看')" width="150" align="center"><template #default="{ row }"><el-checkbox :model-value="row.view || row.inheritedView" :disabled="row.inheritedView" @change="setFieldView(row as FieldPermission, $event)">{{ t('systemRole.view', '查看') }}</el-checkbox></template></el-table-column>
          <el-table-column :label="t('common.edit', '编辑')" width="150" align="center"><template #default="{ row }"><el-checkbox :model-value="row.edit || row.inheritedEdit" :disabled="row.inheritedEdit" @change="setFieldEdit(row as FieldPermission, $event)">{{ t('common.edit', '编辑') }}</el-checkbox></template></el-table-column>
        </el-table>
    ...
    <div v-else>
        <el-radio-group v-model="authorization.dataScope" class="data-scopes">
          <el-radio value="all">{{ t('systemRole.scopeAll', '全部数据') }}</el-radio><el-radio value="dept_and_children">{{ t('systemRole.scopeDeptAndChildren', '本部门及下级') }}</el-radio><el-radio value="dept">{{ t('systemRole.scopeDept', '本部门') }}</el-radio><el-radio value="self">{{ t('systemRole.scopeSelf', '仅本人') }}</el-radio><el-radio value="custom">{{ t('systemRole.scopeCustom', '自定义部门') }}</el-radio>
        </el-radio-group>
    ...
    <div class="role-authorization-panel__footer"><el-button type="primary" :loading="saving" v-perm="'system:role:perm'" @click="save">{{ t('systemRole.saveAll', '保存整套授权') }}</el-button></div>
  </div>
</template>
...
function inheritanceText(sources: AuthorizationSource[]) { return sources.length ? t('systemRole.inheritedFrom', { names: sources.map((source) => source.roleName).join('、') }, '继承自：{names}') : ''; }
...
async function copyAuthorization() {
  ...
  await ElMessageBox.confirm(t('systemRole.copyConfirm', '确认复制所选角色的整套授权吗？'), t('systemRole.copyAuth', '复制授权'), { type: 'warning' }); copying.value = true;

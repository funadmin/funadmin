<template>
  <el-dialog v-model="visible" :title="t('systemConfig.valueDialogTitle', { code: row?.code || '' }, { default: '设置配置值：{code}' })" width="640px" destroy-on-close>
    <el-form label-width="92px">
      <el-form-item :label="row?.remark || t('systemConfig.value', '配置值')">
    ...
    <template #footer>
      ...
      <el-button type="primary" :loading="saving" @click="onSubmit">{{ t('systemConfig.saveValue', '保存配置值') }}</el-button>
    </template>
  </el-dialog>
</template>

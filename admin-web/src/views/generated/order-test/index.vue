...
<div class="flex flex-col gap-4 md:flex-row"><ListCategoryPanel :options="categoryOptions" :model-value="query.__category" @change="onCategory" />    <DataTableShell class="min-w-0 flex-1" storage-key="generated-order-test" :loading="loading" @refresh="loadData">
      ...
      <template #default="{ size, stripe, border, headerCellStyle }"><el-table :data="list" row-key="id" :tree-props="{ children: '__listChildren' }" :size="size" :stripe="stripe" :border="border" :header-cell-style="headerCellStyle" @selection-change="handleSelectionChange">
          ...
          <el-table-column prop="field7" label="多文件上传"><template #default="scope"><span>{{ String(scope.row.field7 ?? '') }}</span></template></el-table-column>
    ...
    </DataTableShell></div><OrderTestForm v-model="dialogVisible" :row="current" @success="loadData" /><OrderTestDetail v-model="drawerVisible" :row="current" />
  </PageWrapper>
</template>

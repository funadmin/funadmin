<template>
  <aside class="source-tree" v-loading="loading" aria-label="业务来源树">
    ...
    <el-button v-if="canReadForm && canMutate && result?.actions.create" @click="open('create')">新增</el-button>
...
import type { FormLeftTreeConfiguration } from '../schema/types';
...
const props = withDefaults(defineProps<{ formKey: string; schemaHash: string; config: FormLeftTreeConfiguration; modelValue?: FormRecordId[]; canReadForm?: boolean; canMutate?: boolean; api?: Pick<typeof formDataApi, 'leftTree' | 'leftTreeForm' | 'mutateLeftTree'> }>(), { canReadForm: true, canMutate: true });
...
const optionsRequest = computed<FormDataSourceRequest>(() => {
  ...
  return async (_key, field, context) => {
    if (!props.canReadForm || sequence !== dialogSequence.value) throw new Error('弹窗已失效');
...
const select = (ids: FormRecordId[]) => { treeRef.value?.setCheckedKeys(ids); treeRef.value?.setCurrentKey(ids[0] ?? null); emit('change', ids); };
...
const open = async (action: 'create' | 'addChild' | 'edit', id: FormRecordId = '') => {
  ...
  if (sequence !== dialogSequence.value) return;
...
const save = async () => {
  if (!props.canMutate || !sourceMeta.value || !result.value?.actions[operation.value] || saving.value || unsupportedValidation.value || !renderer.value) return;
  saving.value = true;
  try {
...
const remove = async (id: FormRecordId) => {
  if (!props.canMutate || !result.value?.actions.delete) return;
...
watch(() => props.modelValue, ids => { treeRef.value?.setCheckedKeys(ids ?? []); treeRef.value?.setCurrentKey(ids?.[0] ?? null); });
</script>
<style scoped>

<template>
  <el-dialog :model-value="modelValue" title="导入 CSV" width="min(760px, 96vw)" destroy-on-close @update:model-value="value => emit('update:modelValue', value)">
    <el-form label-width="90px">
      <el-form-item label="CSV 文件">
        <input type="file" accept=".csv,text/csv" aria-label="选择 CSV 文件" @change="onFile" />
      </el-form-item>
      <el-form-item label="首行表头">
        <el-switch v-model="hasHeader" @change="reparse" />
        <span class="ml-2 text-xs text-[var(--el-text-color-secondary)]">开启时首行按字段名或标签映射列；关闭时按可写字段顺序映射。</span>
      </el-form-item>
    </el-form>
    <el-alert v-if="parseError" :title="parseError" type="error" :closable="false" class="mb-3" />
    <template v-if="rows.length">
      <p class="mb-2 text-sm">已解析 {{ rows.length }} 行（上限 1000 行）；未映射列将被忽略。</p>
    ...
    <template #footer>
      ...
      <el-button type="primary" :disabled="!rows.length || Boolean(parseError)" @click="submit">导入</el-button>
    </template>
  </el-dialog>
</template>
...
function reparse() {
  ...
  if (!table.length) { parseError.value = 'CSV 内容为空或无法解析'; raw.value = []; return; }
  if (table.length > 1001) { parseError.value = '超过 1000 行上限，请拆分后重试'; raw.value = []; return; }
  raw.value = table;
  if (!rows.value.length) parseError.value = '没有可映射的列：表头需使用字段名或字段标签';
}
function submit() {

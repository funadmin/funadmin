<template>
  <el-tabs v-model="tab" type="border-card">
    <el-tab-pane v-if="selectedMeta.kind !== 'layout'" :label="t('formDesigner.tabColumn', '列')" name="column">
      <el-form label-width="90px" size="small" @change="emitUpdate">
        <el-form-item :label="t('formDesigner.fieldName', '字段名')">
          <el-input :model-value="field.field_name" @update:model-value="patch({ field_name: $event })" />
        </el-form-item>
        <el-form-item :label="t('formDesigner.columnType', '列类型')">
          <el-select
        ...
        <el-form-item :label="t('formDesigner.index', '索引')">
          <el-select :model-value="field.index_type" :disabled="sourceType === 'adopted'" class="w-full" @update:model-value="patch({ index_type: $event })">
            <el-option :label="t('formDesigner.none', '无')" value="none" />
            <el-option :label="t('formDesigner.unique', '唯一')" value="unique" />
            <el-option :label="t('formDesigner.normal', '普通')" value="index" />
          </el-select>
        </el-form-item>
        <el-form-item :label="t('formDesigner.comment', '注释')">
          <el-input :model-value="field.comment" @update:model-value="patch({ comment: $event })" />
        </el-form-item>
        <el-form-item :label="t('formDesigner.relationType', '关联类型')">
        ...
        <template v-if="field.relation_type !== 'none'">
          <el-form-item :label="t('formDesigner.relationTable', '关联表')">
            <el-select
              ...
              :placeholder="t('formDesigner.relationTablePlaceholder', '请选择或输入关联表')"
          ...
          <el-form-item :label="t('formDesigner.relationLabelField', '显示字段')">
            <el-select
              ...
              :placeholder="t('formDesigner.relationLabelFieldPlaceholder', '请选择显示字段')"
          ...
          <el-form-item :label="t('formDesigner.relationValueField', '值字段')">
            <el-select
              ...
              allow-create
              :loading="columnsLoading"
              :placeholder="t('formDesigner.relationValueFieldPlaceholder', '请选择值字段')"
              @visible-change="loadRelationColumns"
          ...
          <el-form-item v-if="field.relation_type === 'belongs_to'" :label="t('formDesigner.relationOnDelete', '删除规则')">
    ...
    <el-tab-pane :label="t('formDesigner.tabForm', '表单')" name="form">
      <el-form label-width="90px" size="small">
        <el-form-item :label="t('formDesigner.controlType', '控件类型')">
    ...
    <el-tab-pane v-if="selectedMeta.kind !== 'layout'" :label="t('formDesigner.tabList', '列表')" name="list">
      <el-form label-width="90px" size="small">
        ...
        <el-form-item :label="t('formDesigner.formatter', '格式化器')">
        ...
        <el-form-item :label="t('formDesigner.columnWidth', '列宽')">
          <el-input-number :model-value="field.list_width" :min="0" :max="800" class="w-full" @update:model-value="patch({ list_width: $event ?? 0 })" />
        </el-form-item>
      </el-form>
    </el-tab-pane>
    <el-tab-pane :label="t('formDesigner.tabProperties', '属性')" name="properties">
      <el-form label-width="100px" size="small">
        <el-alert v-if="!propertyFields.length" :title="t('formDesigner.noDynamicProps', '当前组件没有可配置的动态属性')" type="info" :closable="false" class="mb-3" />
      ...
      <el-collapse>
        <el-collapse-item :title="t('formDesigner.rawJson', '原始 JSON（高级）')" name="raw-props">
          <el-form label-width="90px" size="small">
            <el-form-item label="props">
              <el-input :model-value="propsJson" type="textarea" :rows="6" placeholder='{"size":"large"}' @change="onPropsJson" />
            </el-form-item>
            <el-form-item :label="t('formDesigner.optionsSource', '选项来源')">
...
const updatePropertyInput = (property: DynamicPropertyField, value: string) => {
  ...
  try {
    ...
    ElMessage.warning(t('formDesigner.invalidJsonProperty', { title: property.title }, '{title} 需要合法 JSON'));
  }
};
...
const columnLabel = (column: RelationColumn) => `${column.name}${column.comment ? `（${column.comment}）` : ''}${column.primary ? t('formDesigner.primaryKeyTag', ' [主键]') : ''}`;
...
const onPropsJson = (value: string) => {
  try {
    ...
    ElMessage.warning(t('formDesigner.propsJsonInvalid', 'props JSON 不合法'));
  }
};
const onOptionsJson = (value: string) => {
  try {
    ...
    ElMessage.warning(t('formDesigner.optionsJsonInvalid', '选项来源 JSON 不合法'));
  }
};
</script>

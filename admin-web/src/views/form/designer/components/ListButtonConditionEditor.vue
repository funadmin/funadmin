<template>
  <div class="condition-editor condition-grid">
    <el-select :aria-label="t('formDesigner.conditionRuleAria', { label: conditionLabel }, '{label}规则')" :model-value="mode" @change="changeMode">
      <el-option value="none" :label="t('formDesigner.conditionNone', '不限制')" /><el-option value="field" :label="t('formDesigner.conditionField', '字段比较')" />
      <el-option value="and" :label="t('formDesigner.conditionAnd', '全部满足')" /><el-option value="or" :label="t('formDesigner.conditionOr', '任一满足')" /><el-option value="not" :label="t('formDesigner.conditionNot', '取反')" />
    </el-select>
    <template v-if="modelValue && mode === 'field'">
      <el-select :aria-label="t('formDesigner.conditionFieldLabel', '条件字段')" :model-value="modelValue.field" filterable @change="field => patch({ field })"><el-option v-for="field in fields" :key="field" :value="field" :label="field" /></el-select>
      <el-select :aria-label="t('formDesigner.conditionOp', '条件运算')" :model-value="modelValue.op" @change="op => patch({ op, value: ['in', 'notIn'].includes(op) ? [] : '' })"><el-option v-for="op in operators" :key="op" :value="op" :label="op" /></el-select>
      <template v-if="['in', 'notIn'].includes(modelValue.op)">
        <div v-for="(value, index) in values" :key="index" class="value-row"><el-input :model-value="String(value ?? '')" @update:model-value="value => setValue(index, value)" /><el-button @click="removeValue(index)">{{ t('formDesigner.removeValue', '移除值') }}</el-button></div>
        <el-button :disabled="values.length >= 100" @click="patch({ value: [...values, ''] })">{{ t('formDesigner.addValue', '添加值') }}</el-button>
      </template>
      <el-input v-else-if="!['empty', 'notEmpty'].includes(modelValue.op)" :aria-label="t('formDesigner.conditionValue', '条件值')" :model-value="String(modelValue.value ?? '')" :placeholder="t('formDesigner.conditionValuePlaceholder', '数字、布尔、null 自动识别')" @update:model-value="value => patch({ value: scalar(value) })" />
    </template>
    <template v-if="modelValue && ['and', 'or'].includes(mode)">
      <div v-for="(child, index) in modelValue.conditions ?? []" :key="index">
        ...
        <el-button @click="updateChild(index, undefined)">{{ t('formDesigner.removeCondition', '删除条件') }}</el-button>
      </div>
      <el-button :disabled="depth >= 7 || (modelValue.conditions?.length ?? 0) >= 20" @click="patch({ conditions: [...(modelValue.conditions ?? []), { op: 'eq', field: fields[0] ?? '', value: '' }] })">{{ t('formDesigner.addCondition', '添加条件') }}</el-button>
    </template>
...
const { t } = useI18n();
const props = withDefaults(defineProps<{ modelValue?: FormSchemaCondition; fields: string[]; depth?: number; label?: string }>(), { depth: 0 });
const conditionLabel = computed(() => props.label ?? t('formDesigner.conditionDefaultLabel', '条件'));

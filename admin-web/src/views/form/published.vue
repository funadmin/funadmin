...
const openDialog = (row?: Record<string, unknown>) => {
  editingId.value = row ? row[primaryKey.value] as string | number : null;
...
const saveRow = async () => {
  ...
  saving.value = true;
  try {
    if (editingId.value !== null) await formDataApi.update(formKey.value, editingId.value, { ...dialogValues });
...
const removeRow = async (row: Record<string, unknown>) => {
  ...
  await formDataApi.remove(formKey.value, row[primaryKey.value] as string | number);

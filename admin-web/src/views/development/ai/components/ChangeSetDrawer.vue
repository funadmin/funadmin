<template>
  <el-drawer :model-value="modelValue" :title="t('aiDevelopment.changeSet.title')" size="min(920px, 94vw)" @update:model-value="$emit('update:modelValue', $event)">
    <div class="change-set-summary"><el-tag>{{ t('aiDevelopment.changeSet.test', { status: statusLabel(testStatus) }) }}</el-tag><el-tag>{{ t('aiDevelopment.changeSet.security', { status: statusLabel(securityStatus) }) }}</el-tag><span>{{ t('aiDevelopment.changeSet.fileSelectionOnly') }}</span></div>
    ...
    <el-alert v-if="preview?.blocked" :title="t('aiDevelopment.changeSet.conflictBlocked')" type="error" :closable="false" />
    <el-alert v-else-if="!preview" :title="t('aiDevelopment.changeSet.previewRequired')" type="warning" :closable="false" />
    <template #footer>
      <el-button @click="$emit('update:modelValue', false)">{{ t('aiDevelopment.changeSet.close') }}</el-button>
      <el-button type="primary" :disabled="selection.length === 0" @click="$emit('preview', selection)">{{ t('aiDevelopment.changeSet.preview') }}</el-button>

...
describe('管理员认证最终架构契约', () => {
  ...
  it('新 auth、upload、system API 文件与 Attribute 路由继续存在', () => {
    ...
    expect(consoleRoute).not.toContain('auth.AdminAuth/');
    expect(consoleRoute).not.toContain('system.AdminUpload/');
  ...
  it('console 根入口只跳转 admin-web 或返回 404，旧入口统一显式 404', () => {
    const hasRootRedirect = /Route::(?:get|redirect)\s*\(\s*['"]\/?['"][\s\S]{0,160}admin-web/.test(consoleRoute);
    const hasRoot404 = /Route::get\s*\(\s*['"]\/?['"][\s\S]{0,160}\b404\b/.test(consoleRoute);
    expect(hasRootRedirect || hasRoot404, 'console 根入口必须跳转 /admin-web 或返回 404').toBe(true);
    expect(consoleRoute).toMatch(/Route::miss\s*\([\s\S]*\b404\b/);
    expect(consoleRoute).not.toMatch(/['"](?:index|login|ajax)(?:\/[^'"]*)?['"]\s*,\s*['"][^'"]*(?:Index|Login|Ajax)/i);
  });
});

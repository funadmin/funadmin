...
describe('会员分组关联后端契约', () => {
  it('SystemMember 通过关联表过滤并保存分组', () => {
    const source = readProjectFile('../app/admin/controller/system/SystemMember.php');
  ...
  it('SystemMemberGroup 通过关联表检查会员组引用', () => {
    const source = readProjectFile('../app/admin/controller/system/SystemMemberGroup.php');
...
describe('管理员邮箱最大长度契约', () => {
  ...
  it('后端拒绝超过 60 个字符的管理员邮箱', () => {
    const source = readProjectFile('../app/admin/controller/system/SystemAdmin.php');

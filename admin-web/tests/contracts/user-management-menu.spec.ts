...
const migration = readFileSync(
  resolve(process.cwd(), '../database/migrations/009_user_management_menu.sql'),
  'utf8'
);

describe('用户管理菜单层级迁移', () => {
  it('创建独立的用户管理一级目录', () => {
    expect(migration).toContain("'用户管理', '/users'");
    expect(migration).toContain('component=Layout&name=UserManagement&type=M&redirect=/users/member');
    expect(migration).toContain("'admin_web', 'user_management'");
  });
  ...
  it('将系统管理员菜单命名为管理员管理以避免重名', () => {
    expect(migration).toContain("SET `title` = '管理员管理'");

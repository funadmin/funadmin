...
final class BusinessTargetService
{
    ...
    public function __construct(
        ...
        $this->authorized = Closure::fromCallable($authorized ?? static fn (): bool => (new AdminAuthorizationService())->nodeAccess('admin/development.devplugin/options'));
    ...
    {
        $items = [['type' => 'core', 'pluginCode' => null, 'name' => '核心后台', 'scope' => 'admin', 'available' => true, 'reason' => null]];
        if (($this->authorized)()) {
            $states = ($this->states)();
            foreach (($this->options)() as $option) {
                ...
                $items[] = ['type' => 'plugin', 'pluginCode' => $code, 'name' => $option['name'], 'scope' => 'admin',
    ...
    private function unavailableReason(array $option, array $state): ?array
    {
        $code = match (true) {
            ...
            !in_array('admin', $option['scopes'] ?? [], true) => 'ADMIN_MISSING',
    ...
    private function writeBlockReason(array $option): ?string
    {
        ...
        try {
            ...
            foreach ([$directory, $directory . '/plugin.json', $directory . '/app/admin', $directory . '/admin-web'] as $path) {

...
namespace app\admin\ai\infrastructure;
...
final class ContainerAiToolExecutor implements AiToolExecutor
{
    ...
    private const NETWORK_CAPABLE = ['composer', 'npm', 'npx', 'node', 'git'];
    ...
    public function execute(array $call): array
    {
        ...
        $existing = $this->existingCall((string)($call['id'] ?? ''), $context);
        if ($existing !== null) {
            if (($existing['status'] ?? '') !== 'awaiting_approval') throw new RuntimeException('工具调用已消费或不可恢复', 409);
            $name = (string)$existing['tool_name'];
            $arguments = (array)$existing['redacted_arguments'];
        }
        ...
        if ($decision === 'ask' && !$this->approved($definition['operation'], $context, (int) $record['id'])) {
    ...
    private function approved(string $operation, array $context, int $toolCallId): bool
    {
        $conversationId = (int)$context['conversation_id'];
        $adminId = (int)$context['admin_id'];
        $mode = (string)$context['approval_mode'];
        if ($this->approvals->hasSessionApproval($conversationId, $adminId, $operation, $mode)) return true;
        return $this->approvals->consumeOnce($toolCallId, $conversationId, $adminId, $operation, $mode);
    }

    private function existingCall(string $idempotencyKey, array $context): ?array
    {
        if ($idempotencyKey === '') return null;
        foreach ($this->store->toolCalls((int)($context['task_id'] ?? 0)) as $call) {
            if (($call['idempotency_key'] ?? '') === $idempotencyKey
                && (int)($call['conversation_id'] ?? 0) === (int)($context['conversation_id'] ?? 0)) return $call;
        }
        return null;
    }
    ...
    private function validateArgv(array $argv, string $tool): void
    {
        ...
        if (in_array($command, self::NETWORK_CAPABLE, true) && $this->networkIntent($argv)) throw new InvalidArgumentException('容器网络永久禁用，网络依赖必须使用宿主受控代理');
    ...
    private function networkIntent(array $argv): bool
    {
        $joined = strtolower(implode(' ', $argv));
        return preg_match('#https?://|git@|(?:^|\s)(?:install|update|add|fetch|pull|clone|publish|login)(?:\s|$)#', $joined) === 1;
    }
    ...
    private function command(string $name, array $arguments): array
    {
        ...
        return match ($name) {
            ...
            'move' => ['php','-r','if(!rename($argv[1],$argv[2])){exit(1);} ','/workspace/'.$arguments['from'],'/workspace/'.$arguments['to']],
            'delete' => ['php','-r','$p=$argv[1];if(!is_file($p)||!unlink($p)){exit(1);}',$path],

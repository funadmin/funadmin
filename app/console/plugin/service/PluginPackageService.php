...
class PluginPackageService extends AbstractService
{
    ...
    private function validatePlugin(string $directory, string $expectedCode): \app\common\plugin\sdk\Manifest
    {
        $manifest = \app\common\plugin\sdk\Manifest::fromDirectory($directory);
        $code = $manifest->code();
        $this->assertCode($code);
        if ($expectedCode !== '' && strcasecmp($code, $expectedCode) !== 0) {
            throw new RuntimeException('插件包标识与请求标识不一致');

...
namespace app\console\plugin\service\concern;

use app\common\plugin\model\Plugin;
use app\console\plugin\service\PluginInfrastructureService;
use app\console\plugin\service\PluginOperationAuditService;
...
use app\common\plugin\sdk\PluginStorage;
use app\common\plugin\sdk\Registry;
use app\common\plugin\sdk\Service;
...
trait PluginServiceSupport
{
    ...
    private function recordStage(string $code, string $operation, string $stage, ?string $recoveryPath = null): void
    {
        ...
        foreach (\app\common\plugin\sdk\PluginOperationRecorder::stagesThrough($this->operationProgress[$code] ?? 0, $stage) as $candidate => $progress) {

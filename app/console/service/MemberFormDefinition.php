...
final class MemberFormDefinition
{
    public static function build(array $options): array
    {
        ...
        $form->input('email', '邮箱')->default('')->rule('format', 'email')->rule('maxLength', 60)->prop('maxlength', 60)->placeholder('选填');
        ...
        $form->select('tag_ids', '会员标签')->default([])->rule('type', 'array')->rule('maxLength', 32)

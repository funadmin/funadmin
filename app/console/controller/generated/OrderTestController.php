...
use app\console\service\generated\OrderTestService;
use app\console\validate\generated\OrderTestValidate;
...
#[Group('generated/order-test')]
final class OrderTestController extends AdminApiController
{
    use Crud {
  ...
  'id' => 'id',
    ...
    protected function importFields(): array { return array_combine(OrderTestService::WRITABLE_FIELDS, OrderTestService::WRITABLE_FIELDS); }
    protected function importPayload(array $row): array { return (new OrderTestService())->prepareCreatePayload($this->mapImportRow($row)); }
  ...
  9 => 'deletedAt',
    ...
    {
        $payload = array_intersect_key($this->request->post(), array_flip(OrderTestService::WRITABLE_FIELDS));
        return $model === null ? (new OrderTestService())->prepareCreatePayload($payload) : $payload;
    }
    protected function validatePayload(array &$data, ?Model $model = null): ?string
    {
        $validate = new OrderTestValidate();
    ...
    {
        $error = (new OrderTestService())->assertNotReferenced($models, $force);
    ...
    protected function transformData(Model $model): array
    {
        return [
            ...
            'field5' => (int) $model->field_5,
            'field6' => (array) $model->field_6,
            'createdAt' => (string) ($model->created_at ?? ''),
            'updatedAt' => (string) ($model->updated_at ?? ''),
            'deletedAt' => (string) ($model->deleted_at ?? ''),
        ];
    }
    protected function resourceName(): string { return '测试可视化'; }
}

...
final class BusinessTargetService
{
    ...
    public function __construct(
        ...
        $this->options = Closure::fromCallable($options ?? fn (): array => (new DevPluginService($this->root))->options());
    ...
    {
        $items = [['type' => 'core', 'pluginCode' => null, 'name' => '核心后台', 'scope' => 'console']];
        if (($this->authorized)()) {
            $states = ($this->states)();
            foreach (($this->options)() as $option) {
                $code = (string) $option['code'];
                if (!$this->available($states[$code] ?? []) || !$this->writable($option)) continue;
    ...
    public function assertSelection(array $target, string $connection, string $table, bool $generated = false): void
    {
        ...
        if (!in_array($code, array_column($this->candidates()['list'], 'pluginCode'), true)) throw new InvalidArgumentException('BUSINESS_TARGET_UNAVAILABLE');
        $owners = ($this->owners)();
        ...
        if (($target['tableStrategy'] ?? '') === 'owned') {
            if (!str_starts_with($table, 'fun_' . $code . '_')) throw new InvalidArgumentException('BUSINESS_TABLE_PREFIX_REQUIRED');
    ...
    private function available(array $state): bool
    {
        return in_array($state['lifecycle_state'] ?? 'discovered', ['discovered', 'enabled', 'disabled'], true)
            && empty($state['recovery_token']) && empty($state['operation_token']);
    }

    private function writable(array $option): bool
    {
        if (!in_array('console', $option['scopes'] ?? [], true)) return false;
        if (isset($option['businessWritable'])) return $option['businessWritable'] === true;
        try {
            $directory = PathGuard::resolve($this->root, 'plugins/' . $option['code'], '插件目录');
            $data = Manifest::fromDirectory($directory)->toArray();
            if (($data['adminWeb']['source'] ?? '') !== 'admin-web'
                || ($data['migrations']['path'] ?? 'database/migrations') !== 'database/migrations') return false;

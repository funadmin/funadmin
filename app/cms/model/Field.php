...
class Field extends BaseModel
{
    ...
    public static function onAfterWrite($model){
        ...
        if($model->moduleid){
            $cmsmodel = Module::find($model->moduleid);
            $fieledata.='content,';

        }else{
            $cmsmodel = CmsDiyform::find($model->diyformid);
        }
        $fieledata.='member_id,admin_id,status,create_time,update_time,delete_time,';
    ...
    public static function addField($data = [])
    {
        ...
        if($moduleid){
            $tablename = self::get_tablenamebyId($moduleid);
        }else{
            $tablename = self::get_tablenamebyId($diyformid,2);
        }
...
//            }
    ...
    public static function editField($data, $fieldid = 0)
    {
        ...
        if($moduleid) {
            //普通的模型，判断字段是否存在
            if(in_array($data['field'],['id','content','create_time','update_time','delete_time','status','member_id','admin_id'])){
                throw new \Exception("field `" . $data['field'] . "` is already exist");
            }
            //判断表字段名唯一性
            if (self::where('field', $data['field'])->where('moduleid', $moduleid)->value('id')) {
            ...
            if(in_array($data['field'],['id','create_time','update_time','delete_time','status','member_id','admin_id'])){
                throw new \Exception("field `" . $data['field'] . "` is already exist");
            }
            if (self::where('field', $data['field'])->where('diyformid', $diyformid)->value('id')) {
                throw new \Exception("field `" . $data['field'] . "` is already exist");
...
EOF;
    ...
    public function deleteField($fieldid)
    {
        ...
        try {
            ...
            if($moduleid){
                $tablename = self::get_tablenamebyId($moduleid,1);
            }else{
                //获取完整表名
                $tablename = self::get_tablenamebyId($diyformid,2);
            }
...
EOF;
     ...
     * @return string
     */
    public static function  get_tablenamebyId($modiyid,$type=1)
    {
        if($type ==1){

...
class NodeCollection
{
    ...
    public function slot(string $id, string $title, string $slot): FieldBuilder { return $this->layoutNode('text', $id, $title)->slot($slot); }
    public function repeatable(string $field, string $title, string $table, string $valueField, ?callable $children = null): FieldBuilder { return $this->relationContainer('repeatable', $field, $title, $table, $valueField, $children); }
    public function subform(string $field, string $title, string $table, string $valueField, ?callable $children = null): FieldBuilder { return $this->relationContainer('subform', $field, $title, $table, $valueField, $children); }
    ...
    {
        $builder = $this->field($type, $field, $title, '', 'array')->default([])->relation('has_many', $table, '', $valueField, true);
        if ($children !== null) $builder->children($children);
        return $builder;
    }
    ...
    private function id(string $seed): string { return 'node_' . substr(hash('sha256', $seed . ':' . count($this->nodes)), 0, 16); }
}
